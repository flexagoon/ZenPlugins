...
describe('convertTransactions', () => {
  it.each([
    [
      ...
      '12345-card',
      [
        { ID: 'abc-123', Action: 'Market buy', 'Time (UTC)': '2026-01-15 10:30:00+00:00', 'Gross Total': '-1000', 'Currency (Gross Total)': 'EUR' },
        { ID: 'abc-456', Action: 'Market sell', 'Time (UTC)': '2026-01-16 14:20:00+00:00', 'Gross Total': '500', 'Currency (Gross Total)': 'EUR' }
      ] as ExportOperation[],
      [
        {
          hold: false,
    ...
    [
      ...
      '67890-card',
      [
        { ID: 'div-001', Action: 'Dividend (Tax)', 'Time (UTC)': '2026-02-01 09:00:00+00:00', 'Gross Total': '-2.50', 'Currency (Gross Total)': 'USD' },
        { ID: 'div-002', Action: 'Dividend (Dividend)', 'Time (UTC)': '2026-02-01 09:00:00+00:00', 'Gross Total': '25.00', 'Currency (Gross Total)': 'USD' },
        { ID: 'fee-001', Action: 'Stop limit order', 'Time (UTC)': '2026-02-01 09:00:01+00:00', 'Gross Total': '-1.00', 'Currency (Gross Total)': 'USD' }
      ] as ExportOperation[],
      [
        {
          hold: false,
  ...
  it('routes Card debit to card account', () => {
    const operations: ExportOperation[] = [
      { ID: 'card-001', Action: 'Card debit', 'Time (UTC)': '2026-03-01 12:00:00+00:00', 'Gross Total': '-15.50', 'Currency (Gross Total)': 'EUR', 'Merchant name': 'Coffee Shop', 'Merchant category': 'Food & Drinks' }
    ]
  ...
  it('routes Spending cashback to card account when investCashback is false', () => {
    const operations: ExportOperation[] = [
      { ID: 'cashback-001', Action: 'Spending cashback', 'Time (UTC)': '2026-03-01 12:00:00+00:00', 'Gross Total': '0.50', 'Currency (Gross Total)': 'EUR' }
    ]
    const result = convertTransactions(operations, 'acc-1', 'acc-1-card', defaultPreferences)
    expect(result).toHaveLength(1)
    expect(result[0].movements[0].account).toEqual({ id: 'acc-1-card' })
  ...
  it('routes Spending cashback to investment account when investCashback is true', () => {
    const operations: ExportOperation[] = [
      { ID: 'cashback-002', Action: 'Spending cashback', 'Time (UTC)': '2026-03-01 12:00:00+00:00', 'Gross Total': '0.75', 'Currency (Gross Total)': 'EUR' }
    ]
  ...
  it('adds round-up transaction for Card debit when roundUpTransactions is enabled', () => {
    const operations: ExportOperation[] = [
      { ID: 'round-001', Action: 'Card debit', 'Time (UTC)': '2026-04-01 10:00:00+00:00', 'Gross Total': '-12.30', 'Currency (Gross Total)': 'EUR' }
    ]
  ...
  it('does not add round-up when the amount is already whole', () => {
    const operations: ExportOperation[] = [
      { ID: 'round-002', Action: 'Card debit', 'Time (UTC)': '2026-04-01 10:00:00+00:00', 'Gross Total': '-15.00', 'Currency (Gross Total)': 'EUR' }
    ]
    const prefs: Preferences = { ...defaultPreferences, roundUpTransactions: true }
    const result = convertTransactions(operations, 'acc-1', 'acc-1-card', prefs)
